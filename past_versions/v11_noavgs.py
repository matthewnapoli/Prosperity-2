...
class Trader:
    ...
    def order_gen_GIFT_BASKET(self, state):
        ...
        if abs(basket_mid_p-debasket_mid_p) > empty_reserves:
            if basket_mid_p < debasket_mid_p:
                ...
                basket_orders.append(Order("GIFT_BASKET", int(basket_best_ask), int(basket_buy_vol_ext)))
                strawb_orders.append(Order("STRAWBERRIES", int(strawb_highest_bid), int(-strawb_sell_vol_ext)))
                choc_orders.append(Order("CHOCOLATE", int(choc_highest_bid), int(-choc_sell_vol_ext)))
                rose_orders.append(Order("ROSES", int(rose_highest_bid), int(-rose_sell_vol_ext)))
            else:
                logger.print("enter2")
                basket_orders.append(Order("GIFT_BASKET", int(basket_best_bid), int(-basket_sell_vol_ext)))
                strawb_orders.append(Order("STRAWBERRIES", int(strawb_lowest_ask), int(strawb_buy_vol_ext)))
                choc_orders.append(Order("CHOCOLATE", int(choc_lowest_ask), int(choc_buy_vol_ext)))
                rose_orders.append(Order("ROSES", int(rose_lowest_ask), int(rose_buy_vol_ext)))
        elif abs(basket_mid_p-debasket_mid_p) > enter_trade:
            if basket_mid_p < debasket_mid_p:
                logger.print("enter")
                logger.print(basket_buy_vol)
                basket_orders.append(Order("GIFT_BASKET", int(basket_best_ask), int(basket_buy_vol)))
                strawb_orders.append(Order("STRAWBERRIES", int(strawb_highest_bid), int(-strawb_sell_vol)))
                choc_orders.append(Order("CHOCOLATE", int(choc_highest_bid), int(-choc_sell_vol)))
                rose_orders.append(Order("ROSES", int(rose_highest_bid), int(-rose_sell_vol)))
            else:
                logger.print("enter2")
                basket_orders.append(Order("GIFT_BASKET", int(basket_best_bid), int(-basket_sell_vol)))
                strawb_orders.append(Order("STRAWBERRIES", int(strawb_lowest_ask), int(strawb_buy_vol)))
                choc_orders.append(Order("CHOCOLATE", int(choc_lowest_ask), int(choc_buy_vol)))
                rose_orders.append(Order("ROSES", int(rose_lowest_ask), int(rose_buy_vol)))
    ...
    def order_gen_COCONUT(self,state):
        ...
        enter_trade = std_dev_diffs*0.75
        empty_reserves = enter_trade*2
        exit_trade = std_dev_diffs*0.1
        ...
        if abs(coupon_midprice - theo_price) > empty_reserves:
            # if we need to empty reserves
            if coupon_midprice < theo_price:
                coupon_orders.append(Order("COCONUT_COUPON", int(coupon_best_ask), int(coupon_buy_volume_avail_ext if coupon_buy_volume_avail_ext < 2 else 2)))
                coconut_orders.append(Order("COCONUT", int(coconut_best_bid), int(-coconut_sell_volume_avail_ext if -coconut_sell_volume_avail_ext < 1 else -1)))
            elif coupon_midprice > theo_price:
                coupon_orders.append(Order("COCONUT_COUPON", int(coupon_best_bid), int(-coupon_sell_volume_avail_ext if -coupon_sell_volume_avail_ext < -2 else -2)))
                coconut_orders.append(Order("COCONUT", int(coconut_best_ask), int(coconut_buy_volume_avail_ext if coconut_buy_volume_avail_ext < 1 else 1)))
            elif abs(coupon_midprice - theo_price) > enter_trade:
                if coupon_midprice < theo_price:
                    coupon_orders.append(Order("COCONUT_COUPON", int(coupon_best_ask), int(coupon_buy_volume_avail_ext if coupon_buy_volume_avail_ext < 2 else 2)))
                    coconut_orders.append(Order("COCONUT", int(coconut_best_bid), int(-coconut_sell_volume_avail_ext if -coconut_sell_volume_avail_ext < 1 else -1)))
                elif coupon_midprice > theo_price:
        ...
        elif abs(coupon_midprice - theo_price) < exit_trade:
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            #     #careful - trading these is bad in BT, good in live
