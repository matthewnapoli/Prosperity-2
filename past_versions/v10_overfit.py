...
class Trader:
    ...
    def arb_orders_ORCHID(self, state, pos, pos_limit):  
        ...
        logger.print("sBuy @", adj_south_buy_price)
        logger.print("sSell @", adj_south_sell_price)
        bLocal_sSouth_pnl = 0
        sLocal_bSouth_pnl = 0

        #here we buy local (at the ask), and will sell at the `adj_south_sell_price`
        for local_buy_price, buy_volume in local_ask_prices.items():
        ...
        for local_sell_price, sell_volume in local_bid_prices.items():
        ...
        if bLocal_sSouth_pnl<=0 and sLocal_bSouth_pnl <= 0:
            logger.print("no immediate orders")
            _,lowest_local_buy_price,_ = self.calc_metrics_asks(local_ask_prices)
            _,highest_local_sell_price,_ = self.calc_metrics_bids(local_bid_prices)
            logger.print(highest_local_sell_price)
            logger.print(lowest_local_buy_price)
            ...
            logger.print(adj_south_sell_price-lowest_local_buy_price,"?",highest_local_sell_price-adj_south_buy_price)
            if adj_south_sell_price-lowest_local_buy_price < highest_local_sell_price-adj_south_buy_price: 
                #sLocal, bSouth (right side of ineq), gives us a less negative thing, so do it
                return [Order("ORCHIDS", int(adj_south_sell_price+1), -1*sell_volume_avail)]
            else:
                #bLocal, sSouth (left side of ineq), gives us a less negative thing, so do it
                return [Order("ORCHIDS", int(adj_south_buy_price-1), buy_volume_avail)]
            
        elif(bLocal_sSouth_pnl > sLocal_bSouth_pnl):
            buy_orders_to_submit.append(Order("ORCHIDS", int(adj_south_sell_price-4), buy_volume_avail))
            logger.print("buy_orders_to_submit",buy_orders_to_submit)
            return buy_orders_to_submit
        else:
            sell_orders_to_submit.append(Order("ORCHIDS", int(adj_south_buy_price+4), -1*sell_volume_avail))
