...
class Trader:
    ...
    def order_gen_AMETHYSTS_MM(self, order_book, cpos, buy_vol_avail, sell_vol_avail, pos_limit, signal):     
        orders_to_submit: List[Order] = []
        ...
        if buy_volume_avail > 0 and current_pos < 0:
            part = int(floor(buy_volume_avail/2))
            orders_to_submit.append(Order("AMETHYSTS", int(min(undercut_bid+1, buy_signal-1)), part))
            buy_vol_avail-=part
            current_pos+=part
        elif buy_volume_avail > 0 and current_pos > 15:
            part = int(floor(buy_volume_avail/2))
            orders_to_submit.append(Order("AMETHYSTS", int(min(undercut_bid-1, buy_signal-1)), part))
            buy_vol_avail-=part
            current_pos+=part
            ...
            part = int(floor(buy_volume_avail/2))
            orders_to_submit.append(Order("AMETHYSTS", int(bid_am), part))
            buy_vol_avail-=part
            #current_pos+=part

        current_pos = cpos

        if sell_volume_avail > 0 and current_pos > 0:
            part = int(floor(buy_volume_avail/2))
            orders_to_submit.append(Order("AMETHYSTS", int(max(undercut_ask-1, sell_signal+1)), int(-part)))
            sell_vol_avail -= part
            current_pos -= part
        if sell_volume_avail > 0 and current_pos < -15:
            part = int(floor(buy_volume_avail/2))
            orders_to_submit.append(Order("AMETHYSTS", int(max(undercut_ask+1, sell_signal+1)), int(-part)))
            sell_vol_avail -= part
            current_pos -= part
        if sell_volume_avail > 0:
            part = int(floor(buy_volume_avail/2))
            orders_to_submit.append(Order("AMETHYSTS", int(sell_am), int(-part)))
            sell_vol_avail -= part
            current_pos -= part
    ...
    def arb_orders_ORCHID(self, state, pos, pos_limit):
        ...
        if(bLocal_sSouth_pnl > sLocal_bSouth_pnl) or (bLocal_sSouth_pnl == sLocal_bSouth_pnl and bvolume>avolume):
