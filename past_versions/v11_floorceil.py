    ...
    def order_gen_COCONUT(self,state):
        ...
        if abs(coupon_midprice - theo_price) > empty_reserves:
            ...
            if coupon_midprice < theo_price:
                coupon_orders.append(Order("COCONUT_COUPON", coupon_best_ask, coupon_buy_volume_avail_ext))
                coconut_orders.append(Order("COCONUT", coconut_best_bid, -coconut_sell_volume_avail_ext))
            elif coupon_midprice > theo_price:
                coupon_orders.append(Order("COCONUT_COUPON", coupon_best_bid, -coupon_sell_volume_avail_ext))
                coconut_orders.append(Order("COCONUT", coconut_best_ask, coconut_buy_volume_avail_ext))
            ...
            #liquidation (note: if exit = 0, we will automatically liquidate by going short -> long or visa versa) +-2 for liquidity reasons
            if coupon_pos < 0:
                coupon_orders.append(Order("COCONUT_COUPON", coupon_best_ask+2, -coupon_pos))
            elif coupon_pos > 0:
                coupon_orders.append(Order("COCONUT_COUPON", coupon_best_bid-2, -coupon_pos))
            if coconut_pos < 0:
                coconut_orders.append(Order("COCONUT", coconut_best_ask+2, -coconut_pos))
            elif coconut_pos > 0:
                coconut_orders.append(Order("COCONUT", coconut_best_bid-2, -coconut_pos))
